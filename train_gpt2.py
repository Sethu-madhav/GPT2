...
for step in range(max_steps):
    ...
    # for every 250 steps evaluate our validation loss
    if step %  250 == 0:
        model.eval()
        val_loader.reset()
        with torch.no_grad():
    ...
    # sampling
    # for every 250 steps, generate from the model (except step 0, which is noise)
    ...
    if ((step > 0 and step % 250 == 0) or last_step) and (not use_compile):
        ...
        while xgen.size(1) < max_length:  # 8 < 30
            ...
            with torch.no_grad():
                ...
                ix = torch.multinomial(topk_probs, 1, generator=sample_rng) # (B, 1) - (5, 1)
